...
export default function Aspiration() {
  return (
    <section
      className="py-16 md:py-20 lg:py-[120px] bg-navy relative overflow-hidden"
      id="nosotros"
    >
      {/* Background glows */}
      <div
      ...
      <div className="max-w-[1380px] mx-auto px-5 md:px-8 lg:px-[60px] relative z-[1]">
        {/* Header */}
        <div className="mb-10 lg:mb-14">
          ...
          <p className="text-[15px] lg:text-[17px] font-light text-white/55 leading-[1.7] max-w-[640px] mt-4 lg:mt-5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-0.5 mb-12 lg:mb-16 rounded-xl overflow-hidden">
          {paths.map((p) => (
            <div
              key={p.title}
              className="group relative bg-white/[0.04] border border-white/[0.08] p-6 md:p-8 lg:pt-[52px] lg:px-[52px] lg:pb-[56px] transition-colors hover:bg-copper/[0.07] hover:border-copper/25"
            >
              {/* gradient top border on hover */}
              <div className="absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-copper to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />

              <div className="inline-block text-[10px] font-bold tracking-[0.15em] uppercase text-copper-light bg-copper/[0.15] px-3 py-1 rounded-[20px] mb-5 lg:mb-7">
                {p.tag}
              </div>
              <div className="font-heading text-[24px] lg:text-[32px] text-white leading-[1.1] mb-3 lg:mb-4">
                {p.title}
              </div>
              <p className="text-[14px] lg:text-[15px] text-white/50 leading-[1.7] mb-6 lg:mb-9">
                {p.body}
              </p>
              <ul className="flex flex-col gap-2.5 lg:gap-3 mb-7 lg:mb-10">
                {p.items.map((item) => (
                  <li
                    key={item}
                    className="flex items-start gap-3 text-[13px] lg:text-[14px] text-white/50 leading-[1.5]"
                  >
                    <div className="w-[5px] h-[5px] rounded-full bg-copper opacity-70 shrink-0 mt-[7px]" />
                    {item}
                  </li>
                ))}
              </ul>
        ...
        {/* CTA row */}
        <div
          className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between pt-8 lg:pt-10 border-t border-white/[0.08] mb-8 lg:mb-10"
          id="diagnostico"
        >
          <div>
            <h3 className="font-heading text-[24px] md:text-[28px] lg:text-[32px] text-white mb-2 leading-[1.15]">
              ¿Tu empresa tiene la estructura
              <br className="hidden md:block" />{" "}
              <em className="text-white/40">para crecer?</em>
            </h3>
            <p className="text-[14px] lg:text-[15px] text-white/45 leading-[1.5] max-w-[480px]">
        ...
        {/* ISO note */}
        <div className="grid grid-cols-1 md:grid-cols-[auto_1fr] gap-6 md:gap-10 items-start md:items-center px-5 md:px-8 lg:px-10 py-6 lg:py-8 bg-white/[0.05] border border-copper/20 rounded-[10px]">
          <div className="flex flex-row md:flex-col items-center md:items-start gap-3 md:gap-2 shrink-0">
            <div className="font-heading text-[16px] lg:text-[18px] italic text-copper-light border border-copper/50 px-4 md:px-5 py-2 rounded whitespace-nowrap tracking-[0.05em]">
              ISO 9001
            </div>
          ...
          <div className="flex flex-col gap-2.5">
            <p className="text-[13px] lg:text-[14px] text-white/60 leading-[1.55]">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {isoStages.map((stage, i) => (
                <div key={i}>
                  <div
                    key={stage}
                    className="text-[11px] lg:text-[12px] font-semibold text-white/75 bg-white/[0.06] border border-white/[0.1] px-3 md:px-[14px] py-[5px] rounded-[20px] whitespace-nowrap"
                  >
                    {stage}
                  </div>
